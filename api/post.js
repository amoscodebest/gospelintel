...
// 1. ADD: Sanitization helper to prevent broken HTML attributes or XSS vulnerabilities
...
// 2. ADD: Flexible tag update/injection helper
...
export default async function handler(request) {
  ...
  // 3. FIX: Fetch post-template.html to avoid infinite rewrite loops
  const rawHtmlRes = await fetch(`${url.origin}/post-template.html`);
  ...
  // 4. FIX: Prepare and sanitize content strings
  ...
  const rawDescription = post ? stripHtmlAndTruncate(post.content) : DEFAULT_DESC;
  ...
  // 5. FIX: Replace HTMLRewriter with native Regex String Replacement
  ...
  // 6. FIX: Safely construct JSON-LD using unescaped raw values
  const schemaObj = {
  ...
  if (/<script type="application\/ld\+json"[^>]*>.*?<\/script>/is.test(html)) {
    html = html.replace(
      /<script type="application\/ld\+json"[^>]*>.*?<\/script>/is,
      `<script type="application/ld+json" id="schemaStructuredData">${schemaJson}</script>`
    );
  } else {
    html = html.replace(/<\/head>/i, `<script type="application/ld+json" id="schemaStructuredData">${schemaJson}</script>\n</head>`);
  }

  // 7. FIX: Send complete HTML string response
